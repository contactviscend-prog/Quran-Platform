...
import { CompactAttendanceRecorder } from '../shared/CompactAttendanceRecorder';
import { CompactRecitationRecorder } from '../shared/CompactRecitationRecorder';
...
export function TeacherDashboard({ user, organization }: TeacherDashboardProps) {
  ...
  const fetchStats = async () => {
    try {
      ...
      if (!isDemoMode()) {
        toast.error('فشل في تحميل الإح��ائيات');
      }
    } finally {
      setLoading(false);
    }
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <CompactAttendanceRecorder
                organizationId={organization.id}
                teacherId={user.id}
                onViewFullPage={() => setCurrentSection('attendance-recorder')}
              />
              <CompactRecitationRecorder
                organizationId={organization.id}
                teacherId={user.id}
                onViewFullPage={() => setCurrentSection('recitations')}
              />
            </div>
          </div>
        );
